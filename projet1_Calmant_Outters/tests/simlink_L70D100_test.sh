...
echo "(simlink_L70D100_test) limsink 70% de pertes et un délais de 100ms"
./tests/link_sim -p 26000 -P 7777 -l 70 -d 100 -R  &> tests/link.log &
link_pid=$!

# On lance le receiver et capture sa sortie standard
echo "(simlink_L70D100_test) lance le receiver"
...
echo "(simlink_L70D100_test) lance le sender"
if ! ./sender ::1 26000 < tests/input_file 2> tests/sender.log ; then
  echo "(simlink_L70D100_test) Crash du sender!"
...
if kill -0 $receiver_pid &> /dev/null ; then
  echo "(simlink_L70D100_test) Le receiver ne s'est pas arreté à la fin du transfert!"
  kill -9 $receiver_pid
  err=1
else  # On teste la valeur de retour du receiver
  if ! wait $receiver_pid ; then
    echo "(simlink_L70D100_test) Crash du receiver!"
...
if [[ "$(md5sum tests/input_file | awk '{print $1}')" != "$(md5sum tests/received_file | awk '{print $1}')" ]]; then
  echo "(simlink_L70D100_test) Le transfert a corrompu le fichier!"
  echo "(simlink_L70D100_test) Diff binaire des deux fichiers: (attendu vs produit)"
  diff -C 9 <(od -Ax -t x1z tests/input_file) <(od -Ax -t x1z tests/received_file)
  exit 1
else
  echo "simlink_L70D100_test est réussi!"
